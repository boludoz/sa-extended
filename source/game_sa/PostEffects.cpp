...
void CPostEffects::Render() {
    ...
    if (m_bSpeedFX && m_bSpeedFXUserFlag && m_bSpeedFXUserFlagCurrentFrame) {
        ...
        if (m_fSpeedFXManualSpeedCurrentFrame == 0.0f) {
            if (veh && !notsa::contains({ VEHICLE_TYPE_PLANE, VEHICLE_TYPE_HELI, VEHICLE_TYPE_BOAT, VEHICLE_TYPE_TRAIN }, (eVehicleType)veh->GetBaseVehicleType())) {
                bool fxDrawn{};
                if (veh->GetBaseVehicleType() == VEHICLE_TYPE_AUTOMOBILE && veh->handlingFlags.bNosInst && veh->AsAutomobile()->m_fTireTemperature < 0.0f) {
