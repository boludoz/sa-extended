    ...
    m_damageManager{ 0.5f }
{
    ...
    rng::fill(m_aWheelState, WHEEL_STATE_NORMAL);
    rng::fill(m_aWheelRatios, 1.0f);
    rng::fill(m_aRatioHistory, 1.0f);
...
void CAutomobile::ProcessControl()
{
    ...
    if (IsSubPlane() || IsSubHeli()) {
        ...
        if ((carMission == MISSION_PLANE_CRASH_AND_BURN
            ...
            && (m_vecMoveSpeed.z >= 0.0f || m_nPhysicalFlags.bIsInWater))
        {
            bExplodeImmediately = true;
        }
    }

    if (CCheat::IsActive(TAXINITRO_CHEAT)) {
    ...
    if (CCheat::IsActive(NITRO_CHEAT) || bExtraHandlingTaxiBoost) {
    ...
    if (GetStatus() == STATUS_ABANDONED || GetStatus() == STATUS_WRECKED) {
        ...
        if (!vehicleFlags.bVehicleColProcessed
            && m_vecMoveSpeed == 0.0f
            && m_aRatioHistory[CAR_WHEEL_REAR_RIGHT] != 1.0f)
        {
            isVehicleIdle = true;
        }
        ...
        if (IsRealHeli()
            ...
            || CCheat::IsActive(BACKTOTHEFUTURE_CHEAT)
    ...
    for (auto& collisionEntity : m_aGroundPhysicalPtrs) {
        if (collisionEntity) {
    ...
    if (GetStatus() == STATUS_PLAYER && ModelIndices::HasWaterCannon(m_nModelIndex)) {
        ...
        default:
            if (handlingFlags.bHydraulicInst)
                HydraulicControl();
            else if ((CCheat::IsActive(STRONGGRIP_CHEAT) || (extraHandlingFlags & EXTRA_HANDLING_TAXI_BOOST))
    ...
    if (skipPhysics) {
        ...
        bool oldSubmergedInWater = m_nPhysicalFlags.bIsInWater;
        ...
        if (m_nModelIndex == MODEL_SKIMMER) {
            ...
            m_nPhysicalFlags.bForceFullWaterCheck = m_nPhysicalFlags.bIsInWater;
        }
        else {
            ProcessBuoyancy();
        }

        if (!oldSubmergedInWater && m_nPhysicalFlags.bIsInWater) {
        ...
        if (!autoFlags.bIsMonsterTruck) {
            for (int32 i = 0; i < 4; i++) {
                float wheelRadius = 1.0f - m_fSuspensionLength[i] / m_fLineLength[i];
        ...
        for (int32 i = 0; i < 4; i++) {
            if (m_aWheelRatios[i] >= 1.0f) {
                ...
                m_wheelSkidmarkType[i] = static_cast<eSkidmarkType>(g_surfaceInfos.GetSkidmarkType(m_aWheelColPoints[i].m_nSurfaceTypeB));
        ...
        for (int32 i = 0; i < 4; i++) {
            CPhysical* collisionEntity = m_aGroundPhysicalPtrs[i];
            contactSpeeds[i] = GetSpeed(contactPoints[i]);
            if (collisionEntity) {
                contactSpeeds[i] -= collisionEntity->GetSpeed(m_aGroundOffsets[i]);
        ...
        for (int32 i = 0; i < 4; i++) {
            if (m_aWheelRatios[i] >= 1.0f) {
                m_aWheelCounts[i] = std::max(m_aWheelCounts[i] - CTimer::GetTimeStep(), 0.0f);
        ...
        else if (extraPerfectHandling || CCheat::IsActive(STRONGGRIP_CHEAT))
        ...
        if (CCheat::IsActive(STRONGGRIP_CHEAT) || m_nModelIndex == MODEL_VORTEX && vortex->m_fThrottleControl == 0.0f)
            traction *= 4.0f;

        if (this != FindPlayerVehicle() && (extraPerfectHandling || CCheat::IsActive(STRONGGRIP_CHEAT))) {
            traction *= 1.2f;
            acceleration *= 1.4f;
            if (extraHandlingNitros || CCheat::IsActive(STRONGGRIP_CHEAT)) {
                traction *= 1.3f;
                acceleration *= 1.4f;
            }
        }
        ...
        if (speedForward <= 0.01f || m_aWheelCounts[CAR_WHEEL_FRONT_LEFT] <= 0.0f && m_aWheelCounts[CAR_WHEEL_REAR_LEFT] <= 0.0f)
            steerAngle = 1.0f;
        ...
        else if (handlingFlags.bHydraulicInst || CCheat::IsActive(STRONGGRIP_CHEAT) || extraHandlingTaxiBoost)
    ...
    for (int32 i = 0; i < 4; i++) {
        float suspensionChange = m_aRatioHistory[i] - m_aWheelRatios[i];
        ...
        if (m_aWheelRatios[i] < 1.0f && GetStatus() == STATUS_PLAYER) {
            float roughness = (float)g_surfaceInfos.GetRoughness(m_aWheelColPoints[i].m_nSurfaceTypeB) * 0.1f;
        ...
        && std::fabs(m_vecMoveSpeed.z) < 0.0045f)
    {
        if (m_fDamageIntensity <= 0.0f || m_pDamageEntity != FindPlayerPed()) {
            if ((!IsSubPlane() || AsPlane()->m_fThrottleControl == 0.0f) && (!IsSubHeli() || AsHeli()->m_fThrottleControl == 0.0f))
            {
                if (((float)m_wMiscComponentAngle == 0.0f || m_wMiscComponentAngle == m_wMiscComponentAnglePrev)
                    && !m_nPhysicalFlags.bIsInWater)
                {
                    if ((!m_pTowingVehicle || m_pTowingVehicle->m_vecMoveSpeed == 0.0f) && DidAnyWheelTouchGroundPrev()) {
                        if (m_nModelIndex == MODEL_VORTEX || CCheat::IsActive(BACKTOTHEFUTURE_CHEAT)
...
bool CAutomobile::ProcessAI(uint32& extraHandlingFlags) {
    ...
    bool extraPerfectHandling = !!(extraHandlingFlags & EXTRA_HANDLING_PERFECT);
    if (extraPerfectHandling || CCheat::IsActive(STRONGGRIP_CHEAT)) {
        m_vecCentreOfMass.z = m_fSuspensionLength[CAR_WHEEL_FRONT_LEFT] * 0.3f - m_fHeightAboveRoad;
    }
    ...
    if (vehicleFlags.bCanPark
        ...
        && ((CTimer::GetFrameCounter() + static_cast<uint8>(RandomSeed)) & 0xF) == 0)
    {
    ...
    case STATUS_REMOTE_CONTROLLED:
        ...
        if (m_nModelIndex == MODEL_RCBANDIT && !CVehicle::bDisableRemoteDetonationOnContact) {
            ...
            if (RcbanditCheckHitWheels()|| m_nPhysicalFlags.bIsInWater) {
                CRemote::TakeRemoteControlledCarFromPlayer(true);
                BlowUpCar(FindPlayerPed(), false);
            }
        }
        if (FindPlayerInfo().pRemoteVehicle == this)
    ...
    auto carLess3WheelCounter = FindPlayerInfo().nCarLess3WheelCounter;
...
void CAutomobile::ProcessFlyingCarStuff()
{
    if (GetStatus() == STATUS_PLAYER || GetStatus() == STATUS_REMOTE_CONTROLLED || GetStatus() == STATUS_PHYSICS) {
        if (CCheat::IsActive(FLYINGCARS_CHEAT)
...
void CAutomobile::DoHoverSuspensionRatios()
{
    if (GetUp().z >= 0.3f && !vehicleFlags.bIsDrowning) {
        ...
        for (int32 i = 0; i < 4; i++)  {
            ...
            if (m_aWheelRatios[i] < 1.0f) {
                colPointZ = m_aWheelColPoints[i].m_vecPoint.z;
            }
                ...
                && waterLevel > colPointZ && end.z - 1.0f < waterLevel)
            {
                if (waterLevel <= end.z)
                    ...
                    m_aWheelRatios[i] = (start.z - waterLevel) / (start.z - end.z);
                else
                    m_aWheelRatios[i] = 0.0f;

                CVector point = (end - start) * m_aWheelRatios[i];
                point += start;
                point.z = waterLevel;
                m_aWheelColPoints[i].m_vecPoint = point;
                m_aWheelColPoints[i].m_vecNormal = CVector(0.0f, 0.0f, 1.0f);
            }
        }
    }
}
...
void CAutomobile::ProcessSuspension() {
    ...
    for (int32 i = 0; i < 4; i++) {
        springLength[i] = m_aWheelRatios[i];
    ...
    for (int32 wheelLoopIndex = 0; wheelLoopIndex < numWheelLoops; wheelLoopIndex++) {
        ...
        for (int32 i = 0; i < 4; i++) {
            ...
            if (CCheat::IsActive(BACKTOTHEFUTURE_CHEAT) || ModelIndices::IsVortex(m_nModelIndex))
        ...
        for (int32 i = 0; i < 4; ++i) {
            ...
            if (m_aWheelRatios[i] < 1.0f && m_aWheelColPoints[i].m_vecNormal.z > 0.35f) {
                directions[i] = -m_aWheelColPoints[i].m_vecNormal;
            }
        }
        ...
        for (int32 i = 0; i < 4; i++) {
            ...
            contactPoints[i].z -= m_fLineLength[i] * springLength[i];
            contactPoints[i] = GetMatrix().TransformVector(contactPoints[i]);

            auto springTension = 1.f - (m_fSuspensionLength[i] / m_fLineLength[i]);
    ...
    if (forwardSpeed < speedThreshold) {
        float fDir = 0.0f;
        if (m_aWheelRatios[CAR_WHEEL_FRONT_LEFT] == 1.0f
            && m_aWheelRatios[CAR_WHEEL_REAR_LEFT] == 1.0f
            && (m_aWheelRatios[CAR_WHEEL_FRONT_RIGHT] < 1.0f
                || m_aWheelRatios[CAR_WHEEL_REAR_RIGHT] < 1.0f))
        {
            fDir = 1.0f;
        }
            ...
            && m_aWheelRatios[CAR_WHEEL_FRONT_RIGHT] == 1.0f
            && m_aWheelRatios[CAR_WHEEL_REAR_RIGHT] == 1.0f
            && (m_aWheelRatios[CAR_WHEEL_FRONT_LEFT] < 1.0f
                || m_aWheelRatios[CAR_WHEEL_REAR_LEFT] < 1.0f))
        {
            fDir = -1.0f;
        }
...
int32 CAutomobile::ProcessEntityCollision(CEntity* entity, CColPoint* outColPoints) {
    ...
    rng::copy(m_aWheelRatios, wheelColPtsTouchDists.begin());

    const auto originalWheelSuspCompr{m_aWheelRatios};
    ...
    if (   m_nPhysicalFlags.bSkipLineCol
        || m_nPhysicalFlags.bHalfSpeedCollision
        || entity->GetIsTypePed()
    ...
    if (tcd->m_nNumLines) {
        ...
        for (auto i = 0; i < MAX_CARWHEELS; i++) {
            ...
            if (wheelColPtsTouchDist >= 1.f || wheelColPtsTouchDist >= m_aWheelRatios[i]) {
                continue;
            }

            numProcessedWheels++;

            m_aWheelRatios[i] = wheelColPtsTouchDist;
    ...
    if (numProcessedWheels > 0 && entity->GetIsTypeBuilding() && m_pHandlingData->m_fSuspensionHighSpdComDamp > 0.f) {
        for (auto i = 0; i < MAX_CARWHEELS; i++) {
            ...
            const auto compressionNow = std::min(
                ...
                lerp(m_aRatioHistory[i], 1.f, 1.f - m_fSuspensionLength[i] / m_fLineLength[i])
            );
            const auto suspComprDelta = (compressionNow - m_aWheelRatios[i]) * m_fLineLength[i]; // Suspension compression delta (In meters)
            if (suspComprDelta <= 0.1f /* 0x8D3200 */) {
                continue;
            }
            const auto& wheelcp = m_aWheelColPoints[i];
...
void CAutomobile::ProcessControlCollisionCheck(bool applySpeed) {
    ...
    m_nPhysicalFlags.bSkipLineCol = m_nPhysicalFlags.bHalfSpeedCollision = false;

    m_fMovingSpeed = 0.f;
    rng::fill(m_aWheelRatios, 1.f);
...
void CAutomobile::RemoveRefsToVehicle(CEntity* entity) {
    for (auto& ref : m_aGroundPhysicalPtrs) {
        if (ref == entity)
            ref = nullptr;
    }
}
...
void CAutomobile::BlowUpCar(CEntity* pCulprit, bool bInACutscene)
{
    ...
    if (pCulprit == FindPlayerPed() || pCulprit == FindPlayerVehicle()) {
        CWorld::Players[CWorld::PlayerInFocus].HavocCaused += 20;
        CWorld::Players[CWorld::PlayerInFocus].CurrentChaseValue += 10.0f;
...
void CAutomobile::SetupSuspensionLines() {
    ...
    m_fHeightAboveRoad = -fStart + fLength + mi->GetWheelSize(true) * 0.5f;
    ...
    for (auto i = 0u; i < 4u; i++) {
        const auto isFront = (i == CAR_WHEEL_FRONT_LEFT || i == CAR_WHEEL_FRONT_RIGHT);
        m_aWheelSuspensionHeights[i] = -m_fHeightAboveRoad + mi->GetWheelSize(isFront) * 0.5f;
    }
    ...
    if (m_pHandlingData->m_bForceGroundClearance) {
        ...
        const float fMinPos = -m_fHeightAboveRoad + minClearanceOnModel;
        for (auto i = 0u; i < cd->m_nNumSpheres; i++) {
...
void CAutomobile::DoBurstAndSoftGroundRatios()
{
    ...
    for (auto i = 0u; i < 4; i++) {
        const auto& wheelCP = m_aWheelColPoints[i];
        const auto& springLen = m_fSuspensionLength[i];
        const auto& lineLen = m_fLineLength[i];
        auto& compression = m_aWheelRatios[i];
...
void CAutomobile::VehicleDamage(float damageIntensity, eVehicleCollisionComponent collisionComponent, CEntity* damager, CVector* vecCollisionCoors, CVector* vecCollisionDirection, eWeaponType weapon) {
    ...
    if (damageIntensity == 0.f) {
        ...
        // 0x6A7780
        if (m_fDamageIntensity == 0.f || m_nPhysicalFlags.bNotDamagedByCollisions) {
            return;
        }
        ...
        if (m_pDamageEntity) {
            ...
            case eEntityType::ENTITY_TYPE_PED: {
                ...
                if (const auto& ped = *m_pDamageEntity->AsPed(); ped.bIsStanding) {
                    const auto impactDir = DotProduct(ped.m_matrix->GetForward() * ped.m_vecCurrentVelocity.y, m_vecLastCollisionImpactVelocity);
    ...
    if (calcDmgIntensity > minDmgIntensity && GetStatus() != STATUS_WRECKED) {
        ...
        if (GetStatus() == STATUS_PLAYER) {
            FindPlayerInfo().vehicle_time_counter = CTimer::GetTimeInMS();
        }
        ...
        if (calcCollHealthLoss > 0.f) {
            ...
            // 0x6A8338
            if (CCheat::IsActive(VEHICLEOFDEATH_CHEAT) && m_pDamageEntity && m_pDamageEntity == FindPlayerVehicle()) {
...
bool CAutomobile::GetTowBarPos(CVector &vecResult, bool bGenericPosOk, CVehicle *pVeh)
{
    if(m_nModelIndex == eModelID::MODEL_TOWTRUCK || m_nModelIndex == eModelID::MODEL_TRACTOR)
    {
        ...
        vecResult.z = (1.0f - m_wMiscComponentAngle/(float)TOWTRUCK_HOIST_DOWN_LIMIT)*TRACTOR_OFFSET_Z + TOW_LINK_DEFAULT_Z - m_fHeightAboveRoad;
...
void CAutomobile::HydraulicControl() {
    ...
    if (m_wMiscComponentAngle >= 20 || fPrevSpeed <= 0.02f || m_GasPedal == 0.0f) {
        if (m_wMiscComponentAngle && m_wMiscComponentAngle < 61 && fPrevSpeed < 0.01f) {
    ...
    if (driver && driver->GetPadFromPlayer()->HornJustDown()) {
        ...
        if (m_wMiscComponentAngle >= 500) {
            for (int32 i = 0; i < 4; i++) {
                CColLine& line = colData->m_pLines[i];
                wheelPositions[i] = line.m_vecStart.z - m_fLineLength[i] * m_aWheelRatios[i];
                modelInfo->GetWheelPosn(i, pos, false);
                pos.z += extendedUpperLimit;
                line.m_vecStart = pos;
                pos.z -= lineLength;
                line.m_vecEnd = pos;
                m_fSuspensionLength[i] = extendedSpringLength;
                m_fLineLength[i] = lineLength;
                if (m_aWheelRatios[i] < 1.0f) {
                    float compressed = (line.m_vecStart.z - wheelPositions[i]) / lineLength;
                    m_aWheelRatios[i] = std::min(compressed, 1.0f);
                }
            }
            ...
            for (int32 i = 0; i < 4; i++) {
                CColLine& line = colData->m_pLines[i];
                wheelPositions[i] = line.m_vecStart.z - m_aWheelRatios[i] * m_fLineLength[i];
                modelInfo->GetWheelPosn(i, pos, false);
                pos.z += normalUpperLimit;
                line.m_vecStart = pos;
                pos.z -= lineLength;
                line.m_vecEnd = pos;
                m_fSuspensionLength[i] = normalSpringLength;
                m_fLineLength[i] = lineLength;
                if (m_aWheelRatios[i] < 1.0f) {
                    float compressed = (line.m_vecStart.z - wheelPositions[i]) / lineLength;
                    m_aWheelRatios[i] = std::min(compressed, 1.0f);
    ...
    if (!suspensionTriggered) {
        ...
        if (m_wMiscComponentAngle < 500u) {
            ...
            for (int32 i = 0; i < 4; i++) {
                ...
                line.m_vecEnd = pos;
                m_fLineLength[i] = lineLength;
                m_fSuspensionLength[i] = springLength;
                if (m_aWheelRatios[i] < 1.0f) {
                    float compressed = (line.m_vecStart.z - wheelPositions[i]) / lineLength;
                    m_aWheelRatios[i] = std::min(compressed, 1.0f);
                }
    ...
    if (setPrevRatio || suspensionTriggered) {
        for (int32 i = 0; i < MAX_CARWHEELS; i++) {
            const float t = 1.f - m_fSuspensionLength[i] / m_fLineLength[i];
            // same as `(m_aWheelRatios[i] - 1.f + k) / k` where `k = m_fSuspensionLength[i] / m_fLineLength[i]`
            m_aRatioHistory[i] = (m_aWheelRatios[i] - t) / (1.0f - t); 
        }
    }
...
void CAutomobile::TellHeliToGoToCoors(CVector target, float minHeightAboveTerrain, float lowestFlightHeight) {
    ...
    AsHeli()->m_MinHeightAboveTerrain = minHeightAboveTerrain;
    AsHeli()->m_LowestFlightHeight    = lowestFlightHeight;
...
void CAutomobile::SetHeliOrientation(float angle) {
    HeliRequestedOrientation = angle;
}
...
void CAutomobile::DebugCode() {
    ...
    auto* remoteVehicle = FindPlayerInfo().pRemoteVehicle;
...
void CAutomobile::BoostJumpControl() {
    if (m_pDriver && m_pDriver->IsPlayer()) { // TODO: Invert
        if (const auto playerPad = m_pDriver->AsPlayer()->GetPadFromPlayer()) {
            if (playerPad->HornJustDown() && m_aWheelRatios[0] < 1.f) {
...
void CAutomobile::ProcessCarWheelPair(eCarWheel leftWheel, eCarWheel rightWheel, float steerAngle, CVector* contactSpeeds, CVector* contactPoints, float traction, float acceleration, float brake, bool bFront)
{
    ...
    if (m_aWheelCounts[leftWheel] > 0.0f || m_aWheelCounts[rightWheel] > 0.0f) {
        // TOOD: Refactor
        ...
        CVector wheelFwd{};
        CVector wheelRight{};
        if (m_aWheelCounts[leftWheel] > 0.0f) {
            float thrust = driveWheels ? acceleration : 0.0f;

            wheelFwd = GetForward();
            wheelFwd -= DotProduct(wheelFwd, m_aWheelColPoints[leftWheel].m_vecNormal) * m_aWheelColPoints[leftWheel].m_vecNormal;
            wheelFwd.Normalise();
            wheelRight = CrossProduct(wheelFwd, m_aWheelColPoints[leftWheel].m_vecNormal);
            ...
            m_aWheelColPoints[leftWheel].m_nSurfaceTypeA = SURFACE_WHEELBASE;
            float adhesion = g_surfaceInfos.GetAdhesiveLimit(&m_aWheelColPoints[leftWheel]) * traction;
            if (TreatAsPlayerForCollisions())
            {
                adhesion *= g_surfaceInfos.GetWetMultiplier(m_aWheelColPoints[leftWheel].m_nSurfaceTypeB);
                adhesion *= std::min(suspensionBias * m_pHandlingData->m_fSuspensionForceLevel * 4.0f * (1.0f - m_aWheelRatios[leftWheel]), 2.0f);
                if (handlingFlags.bOffroadAbility2 && g_surfaceInfos.GetAdhesionGroup(m_aWheelColPoints[leftWheel].m_nSurfaceTypeB) > ADHESION_GROUP_ROAD)
                {
                    adhesion *= 1.4f;
                }
                else if (handlingFlags.bOffroadAbility)
                {
                    if (g_surfaceInfos.GetAdhesionGroup(m_aWheelColPoints[leftWheel].m_nSurfaceTypeB) > ADHESION_GROUP_ROAD)
                        adhesion *= 1.15f;
                }
            }
        ...
        if (m_aWheelCounts[rightWheel] > 0.0f) {
            float thrust = driveWheels ? acceleration : 0.0f;
            wheelFwd = GetForward();
            wheelFwd -= DotProduct(wheelFwd, m_aWheelColPoints[rightWheel].m_vecNormal) * m_aWheelColPoints[rightWheel].m_vecNormal;
            wheelFwd.Normalise();
            wheelRight = CrossProduct(wheelFwd, m_aWheelColPoints[rightWheel].m_vecNormal);
            wheelRight.Normalise();
            if (canSteer)
            {
                CVector tmp = cosSteerAngle * wheelFwd - sinSteerAngle * wheelRight;
            ...
            m_aWheelColPoints[rightWheel].m_nSurfaceTypeA = SURFACE_WHEELBASE;
            float adhesion = g_surfaceInfos.GetAdhesiveLimit(&m_aWheelColPoints[rightWheel]) * traction;
            if (TreatAsPlayerForCollisions())
            {
                adhesion *= g_surfaceInfos.GetWetMultiplier(m_aWheelColPoints[rightWheel].m_nSurfaceTypeB);
                adhesion *= std::min(suspensionBias * m_pHandlingData->m_fSuspensionForceLevel * 4.0f * (1.0f - m_aWheelRatios[rightWheel]), 2.0f);
                if (handlingFlags.bOffroadAbility2 && g_surfaceInfos.GetAdhesionGroup(m_aWheelColPoints[rightWheel].m_nSurfaceTypeB) > ADHESION_GROUP_ROAD)
                {
                    adhesion *= 1.4f;
                }
                else if (handlingFlags.bOffroadAbility)
                {
                    if (g_surfaceInfos.GetAdhesionGroup(m_aWheelColPoints[rightWheel].m_nSurfaceTypeB) > ADHESION_GROUP_ROAD)
                        adhesion *= 1.15f;
                }
            }
...
// 0x6A7090
void CAutomobile::ProcessCarOnFireAndExplode(bool forceExplode)
{
#define DT_ENGINE_ON_FIRE 225
#define CAR_ON_FIRE_HEALTH (250.0f)
#define DT_ENGINE_BLOW_UP_TIME 5

    uint8 EngineDamage = m_damageManager.GetEngineStatus();
    if (m_fHealth < 250.0f && GetStatus() != STATUS_WRECKED && !vehicleFlags.bIsDrowning)
    {
        int32 triggerFire = 0;

        if (m_pFireParticle == nullptr && (GetVehicleType() != VEHICLE_TYPE_PLANE && GetVehicleType() != VEHICLE_TYPE_HELI))
        {
            triggerFire = 1;
        }

        {
            ...
            {
                if (m_fireParticleCounter == 0)
                {
                    if (IsInAir())
                    {
                        m_fireParticleCounter = 2;
                    }
                    else
                    {
                        m_fireParticleCounter = 1;
                    }
                }
                ...
                else
                {
                    ...
                    if (GetModelIndex() == MODEL_RCBARON ||
                     GetModelIndex() == MODEL_RCRAIDER ||
                      GetModelIndex() == MODEL_RCGOBLIN ||
                       GetModelIndex() == MODEL_RCBANDIT ||
                        GetModelIndex() == MODEL_RCTIGER)
                    {
                        isRC = true;
                    }
                    ...
                    {
                        this->m_fBurnTimer += CTimer::GetTimeInMS();
                    }
                    else
                    {
                        this->m_fBurnTimer += CTimer::GetTimeInMS() * 0.2f;
                    }

                    if (this->m_fBurnTimer > DT_ENGINE_BLOW_UP_TIME * 1000 || vehicleFlags.bIsDrowning)
                    {
                        BlowUpCar(m_pExplosionVictim, false);
                    }
                    else
                    ...
                    if (GetStatus() != STATUS_PLAYER && GetStatus() != STATUS_REMOTE_CONTROLLED)
                    {
                        if (GetModelIndex() != MODEL_RCBARON && GetModelIndex() != MODEL_RCRAIDER && GetModelIndex() != MODEL_RCGOBLIN && GetModelIndex() != MODEL_RCBANDIT && GetModelIndex() != MODEL_RCTIGER)
                        {
                            if (CGeneral::GetRandomNumberInRange(0, 250) < 3)
                            {
                                CExplosion::AddExplosion(this, m_pExplosionVictim, EXPLOSION_ROCKET, this->GetPosition(), 0, 1u, -1.0, 0);
                            }
                        }
                    }
                    else
                    {
                        if (m_pFireParticle == nullptr && m_fBurnTimer > DT_ENGINE_BLOW_UP_TIME * 1000 * 0.5f)
                        {
                            triggerFire = 2;
                        }
                    }
            ...
            {
                this->m_fBurnTimer += CTimer::GetTimeInMS();
                if (this->m_fBurnTimer > DT_ENGINE_BLOW_UP_TIME * 1000)
                {
                    BlowUpCar(m_pExplosionVictim, false);
                }
            }
        ...
        if (triggerFire)
        {
            ...
            CVector            engineOffset = GetDummyPosition(DUMMY_ENGINE); //pModelInfo->GetPosnOnVehicle(VEHICLE_ENGINE_POSN);
            ...
            if (pParentMat)
            {
                if (triggerFire == 1)
                {
                    m_pFireParticle = g_fxMan.CreateFxSystem("fire_car", engineOffset, pParentMat);
                }
                else if (triggerFire == 2)
                {
                    m_pFireParticle = g_fxMan.CreateFxSystem("fire_large", engineOffset, pParentMat);
                }
            }
        ...
        if (m_pFireParticle)
        {
            m_pFireParticle->SetVelAdd(m_vecMoveSpeed * 50.0f);
        }
    }
    else
    {
    ...
    if (EngineDamage > DT_ENGINE_ON_FIRE && this->m_fHealth > CAR_ON_FIRE_HEALTH) {
#if FIX_BUGS // FPS Fix
        this->m_fHealth -= 2.0f * CTimer::GetTimeStepFix();
#else
        this->m_fHealth -= 2.0f;
#endif
    }
...
CObject* CAutomobile::SpawnFlyingComponent(eCarNodes nodeIndex, uint32 collisionType)
{
    ...
    *RwFrameGetMatrix(flyingObjFrame) = *frameLTM; // Set this frame's matrix to be the same as the component's - TODO: This most likely isn't the correct way to do this..
    ...
    obj->m_nObjectFlags.bParentIsACar = true; // bParentIsACar
...
void CAutomobile::ProcessBuoyancy()
{
    ...
    */
    ...
    if (!mod_Buoyancy.ProcessBuoyancy(this, m_fBuoyancyConstant, &vecBuoyancyTurnPoint, &vecBuoyancyForce)) {
        ...
        m_fBuoyancyConstant = m_pHandlingData->m_fBuoyancyConstant;
        for (int32 i = 0; i < 4; ++i) {
            auto& colPoint = m_aWheelColPoints[i];
            if (m_aWheelRatios[i] < 1.0F && g_surfaceInfos.IsWater(colPoint.m_nSurfaceTypeB)) {
    ...
    // 0x6A8F67
    if ((CCheat::IsActive(BACKTOTHEFUTURE_CHEAT) || m_nModelIndex == MODEL_VORTEX)
        && GetStatus() == STATUS_PLAYER
        && GetUp().z > 0.3F
    ) {
...
void CAutomobile::UpdateWheelMatrix(int32 nWheelIndex, int32 nOptionFlags) {
    ...
    if (autoFlags.bIsMonsterTruck
        ...
        const bool bIsHoverMode   = (CCheat::IsActive(BACKTOTHEFUTURE_CHEAT) || GetModelIndex() == MODEL_VORTEX);
        const bool bWheelAirborne = m_aRatioHistory[nWheelNumber] < 1.0f;
        ...
        if (bIsHoverMode && bWheelAirborne && !vehicleFlags.bIsDrowning) {
            ...
            if (!(nOptionFlags & UPDATE_WHEEL_NO_HOVER_EFFECTS)) {
                ...
                CVector pos = m_aWheelColPoints[nWheelNumber].m_vecPoint;
                CVector vel = 0.5f * m_vecMoveSpeed + fRhsWheel * 1.0f * GetRight();

                if (g_surfaceInfos.IsWater(m_aWheelColPoints[nWheelNumber].m_nSurfaceTypeB)) {
                ...
                } else if (g_surfaceInfos.IsSand(m_aWheelColPoints[nWheelNumber].m_nSurfaceTypeB)) {
            ...
            // Usamos m_fHeightAboveRoad (offset 0x7D0) tal como hace el ejecutable original
            float fNewOffset = posn.z + m_fHeightAboveRoad - pModelInfo->GetWheelSize(bFrontWheel) * 0.5f;
...
void CAutomobile::TankControl()
{
    ...
    if (pad->CarGunJustDown()) { // TODO: Denest
        ...
        if (CTimer::GetTimeInMS() > playerInfo.LastTimeBigGunFired + 800) {
            playerInfo.LastTimeBigGunFired = CTimer::GetTimeInMS();
            CVector point;
...
void CAutomobile::BlowUpCarsInPath() {
    ...
    for (auto&& entity : GetCollidingEntities()) {
        ...
        if (ModelIndices::IsRhino(veh.m_nModelIndex))
            continue;
        if (veh.m_nPhysicalFlags.bRenderScorched)
            continue;
...
void CAutomobile::PlaceOnRoadProperly() {
    ...
    // Trace up and down from `point`, keep whichever hit is closer, and drop `point` onto it.
    const auto ProcessPoint = [&](CVector& point, CStoredCollPoly& colPoly) {
        auto foundZ  = 0.0f;
        auto upFound = false;

        if (CWorld::ProcessVerticalLine(point, point.z + 5.0f, colPoint, colEntity, true)) {
            m_pEntityWeAreOn    = colEntity;
            m_bTunnel           = colEntity->m_bTunnel;
            m_bTunnelTransition = colEntity->m_bTunnelTransition;

            foundZ  = colPoint.m_vecPoint.z;
            upFound = true;
        }

        const auto downFound = CWorld::ProcessVerticalLine(point, point.z - 5.0f, colPoint, colEntity, true);
        if (downFound && (!upFound || std::fabs(point.z - colPoint.m_vecPoint.z) < std::fabs(point.z - foundZ))) {
            m_pEntityWeAreOn    = colEntity;
            m_bTunnel           = colEntity->m_bTunnel;
            m_bTunnelTransition = colEntity->m_bTunnelTransition;
    ...
    if (IsSubPlane()) {
        AsPlane()->m_TakeOffDirection = CGeneral::GetATanOfXY(GetForward().x, GetForward().y);
    }
}
...
bool CAutomobile::RcbanditCheck1CarWheels(CPtrListDoubleLink<CVehicle*>& ptrList)
{
    ...
    for (auto* const vehicle : ptrList) {
        ...
        if (distance.x < 10.0f && distance.y < 10.0f) {
            ...
            for (int32 i = 0; i < 4; i++) {
                if (m_aRatioHistory[i] >= 1.0f && GetStatus() != STATUS_SIMPLE)
                    continue;
...
bool CAutomobile::HasCarStoppedBecauseOfLight() {
    ...
    if (!ThePaths.m_pPathNodes[oldNode.Region] || !ThePaths.m_pPathNodes[newNode.Region]) {
        return false;
    }

    const auto& oldPathNode = ThePaths.m_pPathNodes[oldNode.Region][oldNode.Index];
    if (oldPathNode.NumberAdjNodes != 0) {
        const auto* nodeLinks = ThePaths.pAdjacentNodes[oldNode.Region];
        for (uint32 i = 0; i < oldPathNode.NumberAdjNodes; i++) {
            const auto linkIdx = i + oldPathNode.IndexAdjacentNodes;
            if (nodeLinks[linkIdx].Region == newNode.Region && nodeLinks[linkIdx].Index == newNode.Index) {
                const auto naviLinkAddr = ThePaths.pAdjacentLinks[oldNode.Region][linkIdx];
                const auto areaId = naviLinkAddr.Region;
                ...
                if (ThePaths.m_pPathNodes[areaId]) {
                    ...
                    if (carLink.TrafficLightsCycle & 3) {
                        return true;
                    }
                }
            }
    ...
    if (!veryOldNode.IsValid() || veryOldNode.Region == (uint16)-1) {
        return false;
    }

    if (!ThePaths.m_pPathNodes[veryOldNode.Region]) {
        return false;
    }

    const auto& veryOldPathNode = ThePaths.m_pPathNodes[veryOldNode.Region][veryOldNode.Index];
    if (veryOldPathNode.NumberAdjNodes == 0) {
        return false;
    }

    const auto* veryOldNodeLinks = ThePaths.pAdjacentNodes[veryOldNode.Region];
    for (uint32 i = 0; i < veryOldPathNode.NumberAdjNodes; i++) {
        const auto linkIdx = i + veryOldPathNode.IndexAdjacentNodes;
        if (veryOldNodeLinks[linkIdx].Region == oldNode.Region && veryOldNodeLinks[linkIdx].Index == oldNode.Index) {
            const auto naviLinkAddr = ThePaths.pAdjacentLinks[veryOldNode.Region][linkIdx];
            const auto areaId = naviLinkAddr.Region;
            const auto linkId = naviLinkAddr.m_wCarPathLinkId;
...
void CAutomobile::PreRender() {
    ...
    if (vehicleFlags.bVehicleColProcessed && GetVehicleType() != VEHICLE_TYPE_MTRUCK && pColData) {
        ...
        for (auto i = 0; i < 4; i++) {
            ...
            float fHeight = pColData->m_pLines[i].m_vecStart.z;
            if (ratio > 0.0f) {
            ...
            if (m_aWheelRatios[i] < 1.0f && fHeight > m_aWheelSuspensionHeights[i]) {
                ...
                m_aWheelSuspensionHeights[i] += (fHeight - m_aWheelSuspensionHeights[i]) * 0.75f;
            }
        }
    }
    ...
    if (m_nModelIndex != MODEL_DODO && m_nModelIndex != MODEL_VORTEX && m_nModelIndex != MODEL_RCBANDIT) {
        if (GetStatus() == STATUS_SIMPLE) {
            constexpr float WHEEL_SCALE = 1.5f;

            CMatrix matrix;
            CVector posn, pos;

            if (m_aCarNodes[CAR_WHEEL_RB]) {
                matrix.Attach(RwFrameGetMatrix(m_aCarNodes[CAR_WHEEL_RB]), false);
                posn = matrix.GetPosition();
                posn.z = WHEEL_SCALE * m_aWheelSuspensionHeights[CAR_WHEEL_REAR_RIGHT];
                pos = *m_matrix * posn;
                m_aWheelColPoints[CAR_WHEEL_REAR_RIGHT].m_vecPoint = pos;
                m_aWheelColPoints[CAR_WHEEL_REAR_RIGHT].m_nSurfaceTypeB = eSurfaceType::SURFACE_DEFAULT;
            }

            if (m_aCarNodes[CAR_WHEEL_LB]) {
                matrix.Attach(RwFrameGetMatrix(m_aCarNodes[CAR_WHEEL_LB]), false);
                posn = matrix.GetPosition();
                posn.z = WHEEL_SCALE * m_aWheelSuspensionHeights[CAR_WHEEL_REAR_LEFT];
                pos = *m_matrix * posn;
                m_aWheelColPoints[CAR_WHEEL_REAR_LEFT].m_vecPoint = pos;
                m_aWheelColPoints[CAR_WHEEL_REAR_LEFT].m_nSurfaceTypeB = eSurfaceType::SURFACE_DEFAULT;
            }

            if (m_aCarNodes[CAR_WHEEL_RF]) {
                matrix.Attach(RwFrameGetMatrix(m_aCarNodes[CAR_WHEEL_RF]), false);
                posn = matrix.GetPosition();
                posn.z = WHEEL_SCALE * m_aWheelSuspensionHeights[CAR_WHEEL_FRONT_RIGHT];
                pos = *m_matrix * posn;
                m_aWheelColPoints[CAR_WHEEL_FRONT_RIGHT].m_vecPoint = pos;
                m_aWheelColPoints[CAR_WHEEL_FRONT_RIGHT].m_nSurfaceTypeB = eSurfaceType::SURFACE_DEFAULT;
            }

            if (m_aCarNodes[CAR_WHEEL_LF]) {
                matrix.Attach(RwFrameGetMatrix(m_aCarNodes[CAR_WHEEL_LF]), false);
                posn = matrix.GetPosition();
                posn.z = WHEEL_SCALE * m_aWheelSuspensionHeights[CAR_WHEEL_FRONT_LEFT];
                pos = *m_matrix * posn;
                m_aWheelColPoints[CAR_WHEEL_FRONT_LEFT].m_vecPoint = pos;
                m_aWheelColPoints[CAR_WHEEL_FRONT_LEFT].m_nSurfaceTypeB = eSurfaceType::SURFACE_DEFAULT;
            }
        }

        if (GetStatus() == STATUS_PHYSICS || GetStatus() == STATUS_PLAYER || GetStatus() == STATUS_PLAYER_PLAYBACK_FROM_BUFFER || GetStatus() == STATUS_SIMPLE) {
            bool bDoFrontSkidSmoke = (m_aWheelState[CAR_WHEEL_REAR_LEFT] == WHEEL_STATE_SKIDDING || m_aWheelState[CAR_WHEEL_REAR_RIGHT] == WHEEL_STATE_SKIDDING);
            ...
            for (auto i = 0; i < 4; i++) {
                ...
                AddSingleWheelParticles(
                    m_aWheelState[i],
                    m_damageManager.GetWheelStatus((eCarWheel)i),
                    m_aRatioHistory[i],
                    fTempSpeed,
                    &m_aWheelColPoints[i],
                    &m_aWheelColPoints[i].m_vecPoint,
                    fOutsideVector,
                    i,
    ...
    if (m_aCarNodes[CAR_WHEEL_RM]) {
        uint32 nWheelParticleFlags = 0;
        if (m_wheelSkidmarkBloodState[CAR_WHEEL_REAR_LEFT]) {
            nWheelParticleFlags += 1;
        }
        if (m_wheelSkidmarkMuddy[CAR_WHEEL_REAR_LEFT]) {
            nWheelParticleFlags += 2;
        }

        CVector vecColPosition = m_aWheelColPoints[CAR_WHEEL_REAR_LEFT].m_vecPoint + GetForward() * 2.0f;
        AddSingleWheelParticles(
            m_aWheelState[CAR_WHEEL_REAR_LEFT],
            m_damageManager.GetWheelStatus(CAR_WHEEL_REAR_LEFT),
            m_aRatioHistory[CAR_WHEEL_REAR_LEFT],
            fTempSpeed,
            &m_aWheelColPoints[CAR_WHEEL_REAR_LEFT],
            &vecColPosition,
            1.0f,
            5,
            (uint32)m_wheelSkidmarkType[CAR_WHEEL_REAR_LEFT],
            &m_wheelSkidmarkBloodState[CAR_WHEEL_REAR_LEFT],
            nWheelParticleFlags
        );

        nWheelParticleFlags = 0;
        if (m_wheelSkidmarkBloodState[CAR_WHEEL_REAR_RIGHT]) {
            nWheelParticleFlags += 1;
        }
        if (m_wheelSkidmarkMuddy[CAR_WHEEL_REAR_RIGHT]) {
            nWheelParticleFlags += 2;
        }

        vecColPosition = m_aWheelColPoints[CAR_WHEEL_REAR_RIGHT].m_vecPoint + GetForward() * 2.0f;
        AddSingleWheelParticles(
            m_aWheelState[CAR_WHEEL_REAR_RIGHT],
            m_damageManager.GetWheelStatus(CAR_WHEEL_REAR_RIGHT),
            m_aRatioHistory[CAR_WHEEL_REAR_RIGHT],
            fTempSpeed,
            &m_aWheelColPoints[CAR_WHEEL_REAR_RIGHT],
            &vecColPosition,
            1.0f,
            6,
            (uint32)m_wheelSkidmarkType[CAR_WHEEL_REAR_RIGHT],
            &m_wheelSkidmarkBloodState[CAR_WHEEL_REAR_RIGHT],
            nWheelParticleFlags
        );
    }

    if (!CCullZones::CamNoRain() && !CCullZones::PlayerNoRain() && std::abs(fAutomobileSpeed) < 20.0f && CWeather::Rain > 0.02f && CGame::currArea == AREA_CODE_NORMAL_WORLD) {
        AddWaterSplashParticles();
    }

    constexpr float SPEED_LIMIT_NO_EXHAUSTS = 130.0f;
    ...
    case MODEL_COPCARRU:
        if (vehicleFlags.bSirenOrAlarm) {
            ...
            TimeVal = TimeVal & 511;
            if (TimeVal < 100) {
                Red *= (float)TimeVal / 100.0f;
                Green *= (float)TimeVal / 100.0f;
                Blue *= (float)TimeVal / 100.0f;
            } else if (TimeVal > 412) {
                Red *= (512.0f - (float)TimeVal) / 100.0f;
                Green *= (512.0f - (float)TimeVal) / 100.0f;
                Blue *= (512.0f - (float)TimeVal) / 100.0f;
            }
            ...
            for (uint8 SirenNum = 0; SirenNum < 4; SirenNum++) {
                ...
                CVector SirenCoors = ((SirenCoorsL * (float)SirenNum) + (SirenCoorsR * (3.0f - (float)SirenNum))) * 0.333333333f;
                if (SirenState == 0) {
                    CCoronas::RegisterCorona((uint32)this + 21 + SirenNum, this, r1, g1, b1, 255, SirenCoors, 0.4f, 150.0f * TheCamera.m_fLODDistMultiplier, CORONATYPE_SHINYSTAR, FLARETYPE_NONE, CORREFL_NONE, LOSCHECK_OFF, TRAIL_OFF, 0.0f, false, 1.5f, false, 15.0f, false, true);
                } else if (SirenState == 2) {
                    CCoronas::RegisterCorona((uint32)this + 21 + SirenNum, this, r2, g2, b2, 255, SirenCoors, 0.4f, 150.0f * TheCamera.m_fLODDistMultiplier, CORONATYPE_SHINYSTAR, FLARETYPE_NONE, CORREFL_NONE, LOSCHECK_OFF, TRAIL_OFF, 0.0f, false, 1.5f, false, 15.0f, false, true);
                }
            }
        }
        break;
    ...
    case MODEL_FBIRANCH:
        if (vehicleFlags.bSirenOrAlarm) {
            ...
            if ((CTimer::GetTimeInMS() & 256) && (DotProduct(GetForward(), TheCamera.GetForward()) < 0.0f)) {
                CCoronas::RegisterCorona((uint32)this + 21, this, 0, 0, 255, 255, offset, 0.4f, 150.0f * TheCamera.m_fLODDistMultiplier, CORONATYPE_SHINYSTAR, FLARETYPE_NONE, CORREFL_NONE, LOSCHECK_OFF, TRAIL_OFF, 0.0f, false, 1.5f, false, 15.0f, false, true);
            }
        }
        break;
    }

    if (m_nModelIndex != MODEL_RCBANDIT && m_nModelIndex != MODEL_DODO && m_nModelIndex != MODEL_RHINO && (!vehicleFlags.bIsRCVehicle) && GetVehicleAppearance() != VEHICLE_APPEARANCE_HELI) {
        if (m_nModelIndex == MODEL_COMBINE || m_nModelIndex == MODEL_QUAD) {
            DoVehicleLights(*m_matrix, (eVehicleLightsFlags)0);
        } else {
            DoVehicleLights(*m_matrix, (eVehicleLightsFlags)3);
        }
    }
    ...
    auto pModelInfo = (CVehicleModelInfo*)CModelInfo::GetModelInfo(m_nModelIndex);
    ...
    bool bIgnoreRearWheels = (m_pHandlingData->m_nModelFlags & VEHICLE_HANDLING_MODEL_IS_HELI);

    CVector vecFrontForward = m_matrix->TransformVector(CVector(-std::sin(m_fSteerAngle), std::cos(m_fSteerAngle), 0.0f));
    CVector vecRearForward = GetForward();

    if (m_pHandlingData->m_nHandlingFlags & VEHICLE_HANDLING_STEER_REARWHEELS) {
        vecFrontForward = GetForward();
        vecRearForward = m_matrix->TransformVector(CVector(std::sin(m_fSteerAngle), std::cos(m_fSteerAngle), 0.0f));
    } else if ((m_pHandlingData->m_nHandlingFlags & VEHICLE_HANDLING_HB_REARWHEEL_STEER) && m_f2ndSteerAngle != 0.0f) {
        vecRearForward = m_matrix->TransformVector(CVector(-std::sin(m_f2ndSteerAngle), std::cos(m_f2ndSteerAngle), 0.0f));
    }

    for (auto i = 0; i < 4; i++) {
        if (m_aWheelCounts[i] > 0.0f && (!bIgnoreRearWheels || i == CAR_WHEEL_FRONT_LEFT || i == CAR_WHEEL_FRONT_RIGHT)) {
            aWheelOffsets[i] = m_aWheelColPoints[i].m_vecPoint - GetPosition();
            aWheelSpeeds[i] = GetSpeed(aWheelOffsets[i]);
            if (i == CAR_WHEEL_FRONT_LEFT || i == CAR_WHEEL_FRONT_RIGHT) {
                m_wheelSpeed[i] = ProcessWheelRotation(m_aWheelState[i], vecFrontForward, aWheelSpeeds[i], pModelInfo->GetWheelSize(true) * 0.5f);
            } else {
                m_wheelSpeed[i] = ProcessWheelRotation(m_aWheelState[i], vecRearForward, aWheelSpeeds[i], pModelInfo->GetWheelSize(false) * 0.5f);
            }
            m_wheelRotation[i] += m_wheelSpeed[i] * CTimer::GetTimeStep();
        }
    }

    int32 nWheelFlags = (m_nModelIndex == MODEL_RHINO) ? (UPDATE_WHEEL_NO_STEER | UPDATE_WHEEL_NO_TILT | UPDATE_WHEEL_NO_HOVER_TILT) : 0;
    UpdateWheelMatrix(CAR_WHEEL_RF, nWheelFlags);
    UpdateWheelMatrix(CAR_WHEEL_LF, nWheelFlags);
    UpdateWheelMatrix(CAR_WHEEL_RB, nWheelFlags);
    UpdateWheelMatrix(CAR_WHEEL_LB, nWheelFlags);
    if (m_aCarNodes[CAR_WHEEL_RM]) UpdateWheelMatrix(CAR_WHEEL_RM, nWheelFlags);
    if (m_aCarNodes[CAR_WHEEL_LM]) UpdateWheelMatrix(CAR_WHEEL_LM, nWheelFlags);

    if (m_nModelIndex != MODEL_RHINO) {
        ProcessSwingingDoor(CAR_DOOR_LF, eDoors::DOOR_LEFT_FRONT);
        ProcessSwingingDoor(CAR_DOOR_RF, eDoors::DOOR_RIGHT_FRONT);
        ProcessSwingingDoor(CAR_DOOR_LR, eDoors::DOOR_LEFT_REAR);
        ProcessSwingingDoor(CAR_DOOR_RR, eDoors::DOOR_RIGHT_REAR);
        ProcessSwingingDoor(CAR_BONNET, eDoors::DOOR_BONNET);
        ProcessSwingingDoor(CAR_BOOT, eDoors::DOOR_BOOT);

        for (auto& panel : m_panels) {
            if (panel.m_nFrameId != (uint16)-1) {
                panel.ProcessPanel(this, m_aCarNodes[panel.m_nFrameId], m_moveForce, m_turnForce, -1.0f, -1.0f);
            }
        }
        ...
        if (m_swingingChassis.m_doorState == eDoorState::DOOR_HIT_MAX_END && m_aCarNodes[nChassisComponent] != nullptr) {
            ...
            CVector vecDoorOffset = m_matrix->TransformVector(posn);
            m_swingingChassis.Process(this, m_moveForce, m_turnForce, vecDoorOffset);
            orien[(size_t)m_swingingChassis.GetAxes()] = m_swingingChassis.GetDoorAngle();

            matrix.SetRotate(orien[0], orien[1], orien[2]);
            if (m_nModelIndex == MODEL_FIRELA) {
                matrix.RotateX(PACKER_COL_ANGLE_MULT * (float)m_wMiscComponentAngle);
            }
    ...
    case MODEL_BFINJECT: {
        ...
        if (pColData) {
            static const eCarNodes aBuggySuspensionComponents[3] = { CAR_MISC_B, CAR_NODE_NONE, CAR_MISC_C };
            for (int32 i = 0; i < 3; i++) {
                if (aBuggySuspensionComponents[i] != CAR_NODE_NONE && m_aCarNodes[aBuggySuspensionComponents[i]]) {
                    CMatrix matrix;
                    matrix.Attach(RwFrameGetMatrix(m_aCarNodes[aBuggySuspensionComponents[i]]), false);
                    CVector posn = matrix.GetPosition();
                    float fSuspensionWidth = pColData->m_pLines[i].m_vecStart.y - posn.y;
                    float fSuspensionHeight = m_aWheelSuspensionHeights[i] - (pColData->m_pLines[i].m_vecStart.z - m_pHandlingData->m_fSuspensionUpperLimit);
        ...
        SetTransmissionRotation(m_aCarNodes[CAR_MISC_D], m_aWheelSuspensionHeights[CAR_WHEEL_REAR_LEFT], m_aWheelSuspensionHeights[CAR_WHEEL_REAR_RIGHT], posn, false);
        break;
    }
    ...
    case MODEL_DOZER:
        SetComponentRotation(m_aCarNodes[CAR_MISC_A], AXIS_X, DOZER_COL_ANGLE_MULT * (float)m_wMiscComponentAngle, true);
        break;
    case MODEL_CEMENT:
        if (GetStatus() != STATUS_WRECKED) {
            SetComponentRotation(m_aCarNodes[CAR_MISC_A], AXIS_X, DegreesToRadians(-10.0f), true);
            m_fPropRotate += (0.5f + (float)m_wMiscComponentAngle / 2500.0f) * 0.02f * CTimer::GetTimeStep();
            SetComponentRotation(m_aCarNodes[CAR_MISC_B], AXIS_Y, m_fPropRotate, true);
            SetComponentRotation(m_aCarNodes[CAR_MISC_B], AXIS_X, DegreesToRadians(-10.0f), false);
        }
        break;
    case MODEL_PACKER:
        SetComponentRotation(m_aCarNodes[CAR_MISC_A], AXIS_X, PACKER_COL_ANGLE_MULT * (float)m_wMiscComponentAngle, true);
        break;
    case MODEL_TOWTRUCK: {
        float fRotAngle = 0.4f * (float)m_wMiscComponentAngle / 20000.0f;
        SetComponentRotation(m_aCarNodes[CAR_MISC_A], AXIS_X, fRotAngle, true);
    ...
    case MODEL_FORKLIFT:
        if (m_aCarNodes[CAR_MISC_A]) {
            auto pTempFrame = CClumpModelInfo::GetFrameFromId(pModelInfo->GetRpClump(), CAR_MISC_A);
            CVector vecMovedPos = RwFrameGetMatrix(pTempFrame)->pos;
            vecMovedPos.z += FORKLIFT_COL_ANGLE_MULT * (float)m_wMiscComponentAngle;

            CMatrix matrix;
            matrix.Attach(RwFrameGetMatrix(m_aCarNodes[CAR_MISC_A]), false);
            matrix.SetTranslateOnly(vecMovedPos);
            matrix.UpdateRW();
        }
        break;
    case MODEL_COMBINE: {
        if (m_fPropRotate > TWO_PI) {
            m_fPropRotate -= TWO_PI;
        }
        float speedForward = DotProduct(m_vecMoveSpeed, GetForward());
        m_fPropRotate -= std::clamp(speedForward, 0.0f, 0.1f) * CTimer::GetTimeStep();
        SetComponentRotation(m_aCarNodes[CAR_MISC_B], AXIS_X, m_fPropRotate, true);

        CVector wheelPosn;
        pModelInfo->GetWheelPosn(0, wheelPosn, false);
        CMatrix matrix;
        matrix.Attach(RwFrameGetMatrix(m_aCarNodes[CAR_MISC_A]), false);
        CVector posn = matrix.GetPosition();

        float fAveWheelZ = (m_aWheelSuspensionHeights[0] + m_aWheelSuspensionHeights[2]) * 0.5f;
        float fDiffWheelZ = m_aWheelSuspensionHeights[0] - m_aWheelSuspensionHeights[2];

        float fAngX = -std::atan2(fAveWheelZ - wheelPosn.z, posn.y - wheelPosn.y);
        float fAngY = std::atan2(fDiffWheelZ, 2.0f * std::abs(wheelPosn.x));

        SetComponentRotation(m_aCarNodes[CAR_MISC_A], AXIS_X, fAngX + PI, true);
        SetComponentRotation(m_aCarNodes[CAR_MISC_A], AXIS_Y, fAngY, false);
        ...
        float fRate = std::pow(0.95f, CTimer::GetTimeStep());
        m_fDoomVerticalRotation = fRate * m_fDoomVerticalRotation + (1.0f - fRate) * fChassisRollAngle;
    ...
    if (m_nModelIndex == MODEL_SANDKING || (handlingFlags.bHydraulicGeom && handlingFlags.bHydraulicInst && GetBaseVehicleType() == VEHICLE_TYPE_AUTOMOBILE)) {
        ...
        SetTransmissionRotation(m_aCarNodes[CAR_MISC_A], m_aWheelSuspensionHeights[CAR_WHEEL_FRONT_LEFT], m_aWheelSuspensionHeights[CAR_WHEEL_FRONT_RIGHT], posn, true);
        pModelInfo->GetWheelPosn(1, posn, false);
        SetTransmissionRotation(m_aCarNodes[CAR_MISC_B], m_aWheelSuspensionHeights[CAR_WHEEL_REAR_LEFT], m_aWheelSuspensionHeights[CAR_WHEEL_REAR_RIGHT], posn, false);
    }
}
...
void CAutomobile::Render() {
    ...
    if (m_nModelIndex == MODEL_RHINO) {
        if (auto colData = GetColModel()->m_pColData) {
            float ratios[8] = {
                m_doors[4].m_openAngle,
                m_doors[4].m_closedAngle,
                m_doors[4].m_angle,
                m_doors[4].m_prevAngle,
                m_doors[5].m_openAngle,
                m_doors[5].m_closedAngle,
                m_doors[5].m_angle,
                m_doors[5].m_prevAngle,
            };

            for (int32 suspLineOffset = 4; suspLineOffset < 12; ++suspLineOffset) {
                float ratio = ratios[suspLineOffset - 4] - (m_fLineLength[0] - m_fSuspensionLength[0]) / m_fLineLength[0];
                if (ratio < 0.0f) {
                    ratio = 0.0f;
                }
                const auto& line = colData->m_pLines[suspLineOffset];
                CVector translation = (1.0f - ratio) * line.m_vecStart + ratio * line.m_vecEnd;

                RwFrame* wheelNode = (suspLineOffset >= 8) ? m_aCarNodes[CAR_WHEEL_RB] : m_aCarNodes[CAR_WHEEL_LB];
                RwMatrix* modelling = RwFrameGetMatrix(wheelNode);
                translation -= modelling->pos;
    ...
    RwRenderStateSet(rwRENDERSTATEALPHATESTFUNCTIONREF, RWRSTATE(alphaTestRef));

    if (m_nModelIndex != MODEL_RCBANDIT
        && m_nModelIndex != MODEL_DODO
        ...
        && GetVehicleAppearance() != VEHICLE_APPEARANCE_HELI)
    {
        if (m_renderLights.m_bRightFront) {
            CVehicle::DoHeadLightBeam(DUMMY_LIGHT_FRONT_MAIN, *m_matrix, true);
        }
        if (m_renderLights.m_bLeftFront) {
            CVehicle::DoHeadLightBeam(DUMMY_LIGHT_FRONT_MAIN, *m_matrix, false);
        }
    }
}
