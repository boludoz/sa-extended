...
CTaskComplexCarDrive::~CTaskComplexCarDrive() {
    if (m_Veh) {
        if (m_bIsCarSetUp) {
            m_Veh->m_autoPilot.DrivingMode = m_OriginalDrivingStyle;
            m_Veh->m_autoPilot.Mission      = static_cast<eCarMission>(m_OriginalMission);
            m_Veh->m_autoPilot.CruiseSpeed     = m_OriginalSpeed;
        }
