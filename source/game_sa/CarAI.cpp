...
void CCarAI::BackToCruisingIfNoWantedLevel(CVehicle* vehicle) {
    ...
    vehicle->m_autoPilot.Mission = CCullZones::NoPolice()
        ? MISSION_NONE
        : MISSION_CRUISE;
}
...
void CCarAI::CarHasReasonToStop(CVehicle* vehicle) {
    vehicle->m_autoPilot.LastTimeNotStuck = CTimer::m_snTimeInMilliseconds;
}
...
float CCarAI::FindSwitchDistanceClose(CVehicle* vehicle) {
    return (float)vehicle->m_autoPilot.AISwitchToStraightLineDistance;
}
    ...
    for (auto& v : GetVehiclePool()->GetAllValid()) {
        ...
        if (carWithSirenMoveDir2D.Dot(dirToCarWithSiren2D / distToCarWithSiren) <= 0.8f) { // 0x41D8A2
            if (dirToCarWithSiren2D.Dot(CVector2D{ v.GetMoveSpeed() }) < 0.f) { // Going in different directions?
                if (!notsa::contains({ TEMPACT_WAIT, TEMPACT_BRAKE }, vehAP->TempAction)) {
            ...
            if (goingSameDirDot > 0.7f || goingSameDirDot < -0.9f) { // 0x41D8DE
                if (!notsa::contains({ TEMPACT_SWIRVELEFT_STOP, TEMPACT_SWIRVERIGHT_STOP }, vehAP->TempAction)) {
...
void CCarAI::MellowOutChaseSpeed(CVehicle* vehicle) {
    ...
    const auto desiredSpeed    = [&]() -> uint32 {
        ...
        return (uint32)vehicle->m_autoPilot.CruiseSpeed;
    }();
...
void CCarAI::MellowOutChaseSpeedBoat(CVehicle* vehicle) {
    ...
    vehicle->m_autoPilot.CruiseSpeed = []{
        switch (FindPlayerWanted()->GetWantedLevel()) {
...
void CCarAI::TellCarToBlockOtherCar(CVehicle* vehicle1, CVehicle* vehicle2) {
    /*
    vehicle1->m_autoPilot.pTargetEntity = vehicle2;
    CEntity::SafeRegisterRef(vehicle1->m_autoPilot.pTargetEntity);
    ...
    vehicle1->m_autoPilot.SetCruiseSpeed(std::max(vehicle1->m_autoPilot.CruiseSpeed, (uint8)6));
    */
...
void CCarAI::TellCarToFollowOtherCar(CVehicle* follower, CVehicle* toFollow, float radius) {
    follower->m_autoPilot.pTargetEntity = toFollow;
    CEntity::SafeRegisterRef(follower->m_autoPilot.pTargetEntity);

    CCarCtrl::JoinCarWithRoadSystem(follower);

    follower->m_autoPilot.Mission     = MISSION_FOLLOWCAR_FARAWAY;
    follower->vehicleFlags.bEngineOn        = follower->vehicleFlags.bEngineBroken == 0;
    follower->m_autoPilot.CruiseSpeed    = std::max(follower->m_autoPilot.CruiseSpeed, (uint8)6);
    follower->m_autoPilot.FollowCarDistance = (uint8)radius;
}
...
void CCarAI::UpdateCarAI(CVehicle* veh) {
    ...
    if (ap->RecordingNumber >= 0 && (!CVehicleRecording::bUseCarAI[ap->RecordingNumber] || veh->IsSubHeli())) {
        return;
    }

    if (ap->Mission == MISSION_PLANE_ATTACK_PLAYER_POLICE) { // 0x41DAA0
        const auto wntd = FindPlayerWanted();
        if (wntd->GetWantedLevel() < eWantedLevel::WANTED_LEVEL_3 || !FindPlayerVehicle()) {
            ap->TargetCoors = CVector{-6'000.f, -10'000.f, 500.f};
        }
    }
    ...
    case STATUS_GHOST: {
        ...
        { 
            ...
            const auto maxSpeedSq = ap->Mission == MISSION_RAMPLAYER_CLOSE
                ? sq(0.05f)
                : sq(0.04f);
            ...
            if (ap->Mission == MISSION_RAMPLAYER_CLOSE) { // 0x41DDA2
                if (plyrVeh && veh->GetHasCollidedWith(plyrVeh)) { // 0x41DDA2
                    if (!notsa::contains({ TEMPACT_TURNLEFT, TEMPACT_TURNRIGHT }, ap->TempAction)) {
                        ap->SetTempAction(
                            TEMPACT_REVERSE,
        ...
        case MISSION_GOTOCOORDINATES: { // 0x41E2BF - If close enough switch to `MISSION_GOTOCOORDS_STRAIGHT`
            if ((veh->GetPosition() - ap->TargetCoors).SquaredMagnitude2D() <= sq(FindSwitchDistanceClose(veh))) {
                ap->SetCarMission(MISSION_GOTOCOORDINATES_STRAIGHTLINE);
            }
            break;
        }
        case MISSION_GOTOCOORDINATES_STRAIGHTLINE: { // 0x41E3B1
            const auto distToDestSq = (veh->GetPosition() - ap->TargetCoors).SquaredMagnitude2D();

            if (distToDestSq < sq(3.f)) {
                ap->ClearTempAct();
                ap->ClearCarMission();
            }
            else if (distToDestSq >= sq((float)ap->AISwitchToStraightLineDistance + 5.f) && CTimer::GetFrameCounter() % 8 == 0) {
                ap->ClearTempAct();
                ap->SetCarMission(
                    CCarCtrl::JoinCarWithRoadSystemGotoCoors(veh, ap->TargetCoors, true, false)
                    ? MISSION_GOTOCOORDINATES_STRAIGHTLINE
                    : MISSION_GOTOCOORDINATES
                );
            }
        ...
        case MISSION_GOTOCOORDINATES_STRAIGHTLINE_ACCURATE: { // 0x41E473 (Pretty much copy paste `MISSION_GOTOCOORDINATES_STRAIGHTLINE`)
            const auto distToDest2DSq = (veh->GetPosition() - ap->TargetCoors).SquaredMagnitude2D();
            ...
            else if (distToDest2DSq >= sq((float)ap->AISwitchToStraightLineDistance + 5.f) && CTimer::GetFrameCounter() % 8 == 0) {
                ap->ClearTempAct();
                ap->SetCarMission(
                    CCarCtrl::JoinCarWithRoadSystemGotoCoors(veh, ap->TargetCoors, true, false)
                    ? MISSION_GOTOCOORDINATES_STRAIGHTLINE_ACCURATE
        ...
        case MISSION_RAMCAR_FARAWAY: { // 0x41E568 (Pretty much copy-paste `MISSION_GOTOCOORDINATES_STRAIGHTLINE`)
            if (!ap->pTargetEntity) { // Entity was destroyed
                ap->ClearCarMission();
                break;
            }
        ...
        case MISSION_RAMCAR_CLOSE: { // 0x41E642
            ...
            if ((veh->GetPosition() - ap->pTargetEntity->GetPosition()).SquaredMagnitude2D() >= sq(FindSwitchDistanceFar(veh))) { // Inverted
                ap->SetCarMission(MISSION_RAMCAR_FARAWAY);
                CCarCtrl::JoinCarWithRoadSystem(veh);
            }
            else {
                if (veh->GetHasCollidedWith(ap->pTargetEntity) && veh->GetMoveSpeed().SquaredMagnitude() <= sq(0.04f)) {
        ...
        case MISSION_SLOWLY_DRIVE_TOWARDS_PLAYER_1: { // 0x41EB46
            if ((veh->GetPosition() - ap->TargetCoors).SquaredMagnitude2D() <= sq(1.5f)) {
        ...
        case MISSION_ESCORT_FRONT: { // 0x41F771
            if (ap->AISwitchToStraightLineDistance >= 240) {
                break;
            }
        ...
        case MISSION_FOLLOWCAR_CLOSE: { // 0x41EA63
            ...
            if ((veh->GetPosition() - ap->pTargetEntity->GetPosition()).SquaredMagnitude2D() >= sq(FindSwitchDistanceFar(veh))) {
        ...
        default: {
            ...
            if (std::abs(plyrCoors.x - vehPos.x) >= 10.f || std::abs(plyrCoors.y - vehPos.y) >= 10.f) { // 0x41F9B4
                ...
                ap->ClearTempAct();
                ap->DrivingMode = DRIVING_STYLE_AVOID_CARS;
            } else if (ap->Mission == MISSION_CRUISE) { // 0x41F9CE
                if (veh->GetStatus() != STATUS_GHOST) {
                    veh->SetStatus(STATUS_PHYSICS);
                }
    ...
    if (vehMvSpeed2DSq >= sq(0.05f)) {
        ...
        ap->LastTimeNotStuck        = CTimer::GetTimeInMS();
    }

    if (ap->TempAction == TEMPACT_NONE) { // 0x41FB1B
        if (ap->CruiseSpeed == 0 || notsa::contains({ // 0x41FB29
                MISSION_NONE,
            ...
            if (vehMvSpeed2DSq <= sq(0.012f)) { // 0x41FBB6
                ...
                if ((CTimer::GetTimeInMS() - ap->LastTimeNotStuck) > updateInterval) {
                    ap->WhatToTryForReverse = CTimer::GetTimeInMS() >= ap->LastTimeWeStartedTempActReverse + 10'000 // 0x41FC02
                        ? 0
                        : (ap->WhatToTryForReverse + 1) % 4;

                    ap->LastTimeWeStartedTempActReverse = CTimer::GetTimeInMS();

                    if (ap->Mission != MISSION_CRUISE || veh->IsCreatedBy(MISSION_VEHICLE) || (veh->vehicleFlags.bUsedForReplay)) { // 0x41FC4E
                        switch (ap->WhatToTryForReverse) {
                    ...
                    if (veh->IsCreatedBy(RANDOM_VEHICLE)) { // 0x41FCF2
                        ...
                        case DRIVING_STYLE_STOP_FOR_CARS_IGNORE_LIGHTS|DRIVING_STYLE_AVOID_CARS:
                            veh->m_autoPilot.DrivingMode = DRIVING_STYLE_AVOID_CARS;
                            break;
                        }
                    }
    ...
    if (veh->vehicleFlags.bIsLawEnforcer) { // 0x41FDA1
        if (notsa::contains({ MISSION_RAMPLAYER_FARAWAY, MISSION_RAMPLAYER_CLOSE }, ap->Mission)) {
            if (plyrVeh) {
    ...
    if (   notsa::contains({ TEMPACT_NONE, TEMPACT_BOOST_USE_STEERING_ANGLE }, ap->TempAction)
        && notsa::contains(
            {
    ...
    ap->SpeedMultiplier = [&]{
        const auto targetSpeedMult = ap->Mission == MISSION_CRUISE
            ? CCarCtrl::FindSpeedMultiplierWithSpeedFromNodes(ap->SpeedFromNodes)
            : 1.f;
        const auto delta = std::abs(ap->SpeedMultiplier - targetSpeedMult);
        const auto step  = CTimer::GetTimeStep() * 0.01f;
        if (delta < step) {
            return ap->SpeedMultiplier; // No change
        }
        return targetSpeedMult >= ap->SpeedMultiplier
            ? ap->SpeedMultiplier + step
            : ap->SpeedMultiplier - step;
    }();
    ...
    if (CTimer::GetFrameCounter() % 16 == 14 && veh->vehicleFlags.bIsLawEnforcer) {
        if (notsa::contains({ STATUS_SIMPLE, STATUS_PHYSICS }, veh->GetStatus()) && ap->Mission == MISSION_CRUISE && veh->IsSubAutomobile()) {
